...
export default function BattleScreen() {
  ...
  const handleFire = () => {
    if (!selectedEnemyId || !selectedWeapon) return;
  ...
  const doEnemyTurn = async () => {
    ...
    for (const enemy of liveEnemies) {
      ...
      // ✅ Clear state from previous round before this enemy fires
      flushSync(() => {
        setFiringTankId(null);
    ...
    <div className="min-h-screen bg-black text-white font-mono p-4 flex flex-col items-center">
              ...
              <div className="text-center mt-1 text-sm">
                ...
                <div className="w-24 h-2 bg-gray-700 rounded overflow-hidden my-1">
                  <div
                    ...
                    style={{ width: `${(tank.hp / tank.maxHp) * 100}%` }}
                  ></div>
                </div>
                <p className="text-xs">
      ...
      <div className="mt-4 text-center">
        ...
        <div className="flex justify-center gap-3 flex-wrap">
          {[
            { label: "Machine Gun", key: "machinegun", color: "bg-blue-600 hover:bg-blue-700" },
            { label: "Cannon", key: "cannon", color: "bg-orange-600 hover:bg-orange-700" },
            { label: "Missile", key: "missile", color: "bg-purple-600 hover:bg-purple-700" },
            { label: "Airstrike", key: "airstrike", color: "bg-red-600 hover:bg-red-700" },
          ].map((weapon) => (
            <button
              key={weapon.key}
      ...
      <div className="mt-4 text-center">
        <button
          onClick={handleFire}
          disabled={!selectedEnemyId || !selectedWeapon || enemyTurnActive}
          className={`px-5 py-2 rounded font-bold mt-2 ${
            !selectedEnemyId || !selectedWeapon || enemyTurnActive
